...
playsnd() {
	if [ "$2" = duck ]; then
		PULSE_PROP='media.role=event' mplayer $1 </dev/null >/dev/null 2>&1
	elif [ "$2" = mute ]; then
		pasuspender -- mplayer -ao alsa:device=plughw=0.0 $1 </dev/null >/dev/null 2>&1
	else
		PULSE_PROP='media.role=other' mplayer $1 </dev/null >/dev/null 2>&1
	fi
}
